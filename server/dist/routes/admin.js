...
import { authenticate, requirePermission, requireAdmin } from '../middlewares/auth.js';
...
adminRoutes.put('/admin/plans/:id', async (c) => {
    ...
    if (data.price !== undefined)
        data.price = Number(data.price);
    if (data.voucherCount !== undefined)
        data.voucherCount = Number(data.voucherCount);
...
adminRoutes.get('/admin/metrics', requirePermission('view_metrics'), async (c) => {
    ...
    enrollments.forEach(e => {
        ...
        if (!monthlyMap[m])
            monthlyMap[m] = { month: m, matriculas: 0, concluidos: 0, receita: 0 };
        monthlyMap[m].matriculas++;
        if (e.status === 'completed')
            monthlyMap[m].concluidos++;
    });
    orders.forEach(o => {
...
adminRoutes.put('/admin/settings', async (c) => {
    ...
    let settings = await prisma.systemSettings.findFirst();
    const data = { activeGateway, googleClientId, googleClientSecret };
