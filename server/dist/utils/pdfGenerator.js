...
export async function generateCertificatePDF(certificate, template, user, course) {
    ...
    for (const el of elements) {
        let textToDraw = '';
        if (el.type === 'student_name')
            textToDraw = user.name;
        else if (el.type === 'course_name')
            textToDraw = course.title;
        ...
        else if (el.type === 'workload')
            textToDraw = course.duration || '0 horas';
        else if (el.type === 'validation_code')
            textToDraw = `Código de Validação: ${certificate.code}`;
