...
export const classroomRoutes = new Hono<{ Bindings: Bindings, Variables: { user: UserPayload } }>();
...
classroomRoutes.post('/enrollments/:id/quiz', async (c) => {
  ...
  if (!moduleId || !answers) return c.json({ error: 'moduleId and answers are required' }, 400);

  const module = await prisma.module.findUnique({ where: { id: Number(moduleId) } });
  if (!module || module.type !== 'quiz') return c.json({ error: 'Invalid module' }, 400);
